...
export const calculateProbabilityByTurn = (
  ...
  const landCountForTurn = deck.cards
    .filter(
      (card) =>
...
export const analyzeDeckConsistency = (deck: {
  ...
  const deckWithLands = {
    cards: deck.cards
      .filter((card) => !card.name.includes('Mountain') && !card.name.includes('Island'))
      .map((card) => ({
        name: card.name,
    ...
    lands: deck.cards
      ...
      .map((card) => ({
        name: card.name,
        produces: card.name.includes('Mountain')
          ? ['R']
          : card.name.includes('Island')
            ? ['U']
            : card.name.includes('Plains')
  ...
  try {
    ...
    return {
      ...
      colorBalance: {
        balanced: {
          sources: totalLands,
          required: Math.ceil(deck.totalCards * 0.4),
          ratio: landRatio,
        },
      },
      hybridManaHandling:
        deckWithLands.cards.some(
          (card) =>
...
export const calculateOptimalLandCount = (deck: {
  cards: Array<{ name: string; manaCost?: ManaCost | string; cmc?: number; quantity: number }>
  format?: string
  averageCMC?: number
  deckSize?: number
