import type { Card, DeckCard, ColorRequirement, TurnAnalysis, SimulationParams, SimulationResult } from '@/types'
...
export const hypergeometric = (
  ...
  const favorable = combinations(successes, target) * combinations(population - successes, draws - target)
...
export const analyzeCard = (
  ...
  const colorSources = colors.reduce((acc, color) => {
    const sources = lands.reduce((count, land) => {
      if (land.card.producedMana?.includes(color)) {
  ...
  for (let turn = 1; turn <= 6; turn++) {
    ...
    for (const color of colors) {
      const required = calculateRequiredSources(card.manaCost!, turn)
...
export const getLandsProducingColor = (lands: DeckCard[], color: ManaColor): DeckCard[] => {
  return lands.filter(land => 
    land.card.producedMana?.includes(color) ||
...
export const calculateOptimalColorDistribution = (
  ...
  spells.forEach(spell => {
    const colors = extractColors(spell.card.manaCost || '')
    colors.forEach(color => {
      const symbols = parseManaCost(spell.card.manaCost || '')
