...
import { Box, Typography, CircularProgress, Skeleton, useTheme, Alert } from '@mui/material'
  ...
  return (
    <div style={style}>
      ...
      >
        ...
        <Typography variant="body2">{analysis.cardsDrawn} cards</Typography>
        <Typography variant="body2" fontWeight="bold" color={probabilityColor}>
...
export const VirtualizedTurnAnalysis = memo<VirtualizedTurnAnalysisProps>(
  ({ analyses, height = 400, onRowClick }) => {
...
const LazyChart = memo<LazyChartProps>(({ data, type, loading = false }) => {
  ...
  useEffect(() => {
    ...
    const loadChart = async () => {
      try {
        ...
        const Component = type === 'line' ? LineChart : type === 'bar' ? BarChart : PieChart
  ...
  if (!ChartComponent) {
    return <Alert severity="error">Failed to load chart component</Alert>
  }
...
export const ProbabilityDisplay = memo<ProbabilityDisplayProps>(
  ({ probability, label = 'Probability', precision = 1 }) => {
    const theme = useTheme()
    ...
    return (
      <Box>
        <Box display="flex" gap={3} mb={3}>
          <ProbabilityDisplay probability={result.successRate / 100} label="Success Rate" />
          <Box textAlign="center">
        ...
        <LazyChart data={chartData} type="bar" />
      </Box>
    )
  }
)
...
export const PerformanceMonitor = memo<PerformanceMonitorProps>(
  ({
    ...
    return (
      <>
        {children}
        {import.meta.env.DEV && renderTime && renderTime > threshold && (
          <Box
            position="fixed"
            top={10}
            right={10}
            ...
            zIndex={9999}
          >
            <Typography variant="caption">Slow render: {renderTime.toFixed(2)}ms</Typography>
          </Box>
        )}
      </>
    )
...
export const DebouncedInput = memo<DebouncedInputProps>(
  ({ value, onChange, delay = 300, placeholder }) => {
