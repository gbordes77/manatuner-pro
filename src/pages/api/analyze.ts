...
export default async function handler(
  ...
  try {
    ...
    const landCount = 24;
    const handSize = 7;
    const onThePlay = analysisParams?.playFirst ?? true;

    const deckConfiguration: DeckConfiguration = {
      totalCards: deckSize,
      landCount: landCount,
      colorRequirements: [],
      averageCMC: 3.0,
      format: (format as any) || "standard",
      fetchlands: 0,
      duallands: 0,
      basics: landCount,
    };

    // Color sources map for analysis
    ...
    // Default color requirements based on deck analysis
    const colorRequirements: ColorRequirement[] = [];

    const analysisOptions = {
      enableMonteCarlo: false,
      enableMultivariate: false,
      maxTurn: analysisParams?.targetTurn || 6,
      iterations: analysisParams?.iterations || 10000,
    };

    const { enableMonteCarlo, enableMultivariate, maxTurn, iterations } =
      analysisOptions;
    ...
    for (const requirement of colorRequirements) {
      const { color, intensity } = requirement;
      const sourcesInDeck = sources[color] || 0;
    ...
    const allTurnAnalysis: TurnAnalysis[] =
      Object.values(turnAnalysisResults).flat();

    // Run Monte Carlo simulation if requested
    let monteCarloResult: MonteCarloResult | undefined;
    if (enableMonteCarlo) {
      const monteCarloParams = {
        deckSize: deckConfiguration.totalCards,
        landCount: deckConfiguration.landCount,
        targetTurn: maxTurn,
        iterations,
        mulliganStrategy: (analysisParams?.mulliganStrategy ||
          "conservative") as "none" | "aggressive" | "conservative" | "optimal",
        playFirst: analysisParams?.playFirst ?? true,
        maxMulligans: analysisParams?.maxMulligans || 2,
      };

      monteCarloResult =
        await advancedMathEngine.runMonteCarloSimulation(monteCarloParams);
    }
    ...
    for (const [color, turnAnalysis] of Object.entries(turnAnalysisResults)) {
      const problematicTurns = turnAnalysis.filter(
        (analysis) => analysis.castProbability < 0.9,
      );
    ...
    const overallRating =
      worstProbability >= 0.95
        ? "excellent"
        : worstProbability >= 0.9
          ? "good"
          : worstProbability >= 0.8
            ? "acceptable"
            : worstProbability >= 0.6
              ? "poor"
              : "unplayable";

    const criticalTurn =
      allTurnAnalysis.find((analysis) => analysis.castProbability < 0.9)
        ?.turn || null;
