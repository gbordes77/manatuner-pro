...
> **Heads-up (2026-04-18)**: this document was generated by the BMAD
> `document-project` workflow on 2026-01-06 as a baseline snapshot. It has
> drifted since then. Notable outright-false items below (kept for
> historical contrast, not as current reference):
>
> - `src/hooks/useMonteCarloWorker.ts` — **deleted** in v2.5.5 (dead code).
> - `src/services/supabase.ts` — **deleted** on 2026-04-13 (Supabase purge).
> - `src/store/slices/` — only `analyzerSlice.ts` survives; `deck`, `analysis`, `auth`, `ui` slices were deleted in SPRINT 1 (pre-v2.1.0).
> - `utils/mathsReferences.ts` — Karsten tables moved to `src/types/maths.ts` (single source of truth, 2026-04-06 audit).
> - `utils/manaCostParser.ts` added in v2.5.5 (exports `countPipsInCost` + `KarstenColor`).
> - `src/components/analyzer/QuickVerdict.tsx` added in v2.5.5.
> - Sample decks now include `edh` (Atraxa Superfriends, 100c, v2.5.6) and
>   `limited` (Selesnya draft, 40c, v2.5.7) in addition to the three
>   Constructed archetypes.
>
> For current truth, rely on `git ls-files src/` + `docs/ARCHITECTURE.md`
>
> - `CHANGELOG.md` + `CLAUDE.md`. Rebuild this doc only if you need a
>   fresh BMAD snapshot — otherwise it's a January 2026 artifact.
